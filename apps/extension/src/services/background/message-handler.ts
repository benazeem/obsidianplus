...
export async function handleMessage(
  ...
  if (!ctx.hostLaunched && !ctx.webSocket.isConnected()) {
    if (message.type !== 'LAUNCH_HOST' && message.type !== 'PING_HOST') {
      sendResponse({
        success: false,
        error:
  ...
  try {
    ...
    switch (message.type) {
      ...
      case 'SAVE_OBSIDIAN_FILE':
        response = await saveMarkdown(ctx, {
      ...
      case 'DISCONNECT_HOST':
        ...
        ctx.nativeMessaging?.disconnect()
        ctx.hostLaunched = false
        response = await disconnectHost(ctx)
        break
...
async function disconnectHost(ctx: BackgroundContext) {
  const msg = { type: 'shutdown', payload: {}, timestamp: Date.now() }
